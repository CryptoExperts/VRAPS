...
####################### No Batching Version #######################
def verification_random_probing_exp_2_(indices, indices_o, weights, exps,  exps_str, secret_deps, random_deps, nb_occs, coeff_max, nb_shares, t, verbosity, copy = False):
    if(t >= nb_shares):
        print("t (= " + str(t) +  ") >= nb_shares (= " + str(nb_shares) + ")")
        exit()
    if((len(secret_deps[0]) != 1) and (len(secret_deps[0]) != 2)) :
    ...
    if(nb_inputs > 1):
        ...
        #    if((coeff_c_I1[c] + coeff_c_I2[c] - coeff_c_I1_and_I2[c]) != coeff_c_I1_or_I2[c]):
        #        print("!!!!!!!!!!!!!!! Error EXP2 on coefficient " + str(c))
        #        exit()
        return coeff_c_I1, coeff_c_I2, coeff_c_I1_and_I2, coeff_c_I1_or_I2
    else:
        if(verbosity == 2):
